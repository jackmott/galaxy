...
public class Table : UIBehaviour, ILayoutGroup {
  ...
  public void reset() {
    ...
    if (this.headerRect != null) Destroy(this.headerRect.gameObject);
    if (this.footerRect != null) Destroy(this.footerRect.gameObject);
    if (this.columnOverlay != null) Destroy(this.columnOverlay.gameObject);
  ...
  public void fadeOverlay(float overTime, float v0, float v1, float delay=0) {
    if (this.overlayIsHiding ||
      !this.loadingOverlay.gameObject.activeInHierarchy) return;
  ...
  public void setBodyRectSizeLater(float s1, float s2) {
    StopCoroutine("_setBodyRectSizeLater");
    StartCoroutine(this._setBodyRectSizeLater(s1, s2));
  }

  IEnumerator _setBodyRectSizeLater(float s1, float s2) {
    yield return new WaitForEndOfFrame();
