...
public class Column {
  ...
  public enum ColumnType { TEXT, IMAGE };
  public enum HorAlignment { LEFT, CENTER, RIGHT };
  ...
  public Color? headerIconColor {
    ...
    set {
      if (!this.table.hasHeaderIcons) return;
  ...
  public int calcFont(bool isHeader, bool isFooter) {
    if (!isHeader && !isFooter)
      return Math.Max(this.rowFontSize, this.table.defaultFontSize);
    else if (isHeader)
      return Math.Max(this.headerFontSize, this.table.defaultFontSize);
    else
      return Math.Max(this.footerFontSize, this.table.defaultFontSize);
  }
